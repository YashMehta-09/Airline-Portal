...
public class TicketDetailsDAO implements TicketDAO {
	...
	public Ticket getTicketDetails(int ticketId) {
		...
		/*
		 * for(User t:list) { System.out.println(t.getStatus());
	...
	public boolean updateTicket(String tstatus, int ticketId, float payment) {
		...
		/*
		 * Query qry =
		 * session.createQuery("update Ticket t set t.status=?1 where t.ticketId=?2");
		 * qry.setParameter(1,tstatus); qry.setParameter(2,ticketId); int res =
		 * qry.executeUpdate();
		 * 
		 * if(res>0) { status=true; System.out.println(res); }
		...
		/*
		 * String hql =
		 * "UPDATE Ticket set status = :status ,payment=:payment   WHERE ticketId = :ticketId"
		 * ; Query query = session.createQuery(hql);
		 ...
		 * result); if(result>=0) { status=true; }
		 */
		return status;
	}
}
